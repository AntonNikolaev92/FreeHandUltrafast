...
Resource.RcvBuffer(1).rowsPerFrame = nLines*P.endDepth*2*2*2*nFrames; % this size allows for all rays, with range of up to 400 wvlngths
...
Receive = repmat(struct('aperture', 1, ...
    ...
    'callMediaFunc', 0),1,nLines*nFrames*nPackages);

% - Set event specific Receive attributes for each frame.
for iPackage = 1:nPackages
    startPackage = (iPackage-1)*nFrames*nLines;
    for iFrame = 1:nFrames
        startFrame = (iFrame - 1)*nLines;
        Receive(startPackage+startFrame+1).callMediaFunc = 1;
        for iLine = 1:nLines
            Receive(startPackage + startFrame + iLine).aperture = TX(iLine).aperture; % mux aperture same as transmit
            Receive(startPackage + startFrame + iLine).Apod(1:128) = 1.0;
            Receive(startPackage + startFrame + iLine).framenum = iPackage;
            Receive(startPackage + startFrame + iLine).acqNum = startFrame + iLine;
        end
    end
end
...
for iPackage = 1:nPackages
    ...
    for iFrame = 1:nFrames
        ...
        for iLine = 1:nLines        % Acquire frame
            ...
            Event(n).rcv = startPackage + startFrame + iLine;
...
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = scRetToMatlab;
n = n+1;
