...
%    central porti
% on of the transducer aperture where the full 128 channel
...
PData.Origin = [-Trans.spacing*(P.numRays-1)/2,0,P.startDepth]; % x,y,z of upper lft crnr.
...
firstRayLocX = -((nLines-1)/2)*Trans.spacing;
for i = 1:P.numRays
...
Resource.RcvBuffer(1).rowsPerFrame = nLines*P.endDepth*2*2*2*nFramesWP; % this size allows for all rays, with range of up to 400 wvlngths
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = nPackages;
...
TX = repmat(struct('waveform', 1, ...
  ...
  'focus', P.txFocus, ...
  'Steer', [0.0,0.0], ...
  'Delay', zeros(1,Resource.Parameters.numTransmit)), 1, nLines);
...
for n = 1:nLines  % specify P.numRays transmit events
...
Receive = repmat(struct('aperture', 1, ...
  ...
  'sampleMode', 'BS100BW', ...
  'mode', 0, ...
  'samplesPerWave', 2, ... % Sampling frequcy is fs= 2f
  'callMediaFunc', 0),1,nLines*nFramesWP*nPackages);

% - Set event specific Receive attributes for each frame.
for iPackage = 1:nPackages
  startPackage = (iPackage-1)*nFramesWP*nLines;
  for iFrameWP = 1:nFramesWP
    startFrameWP = (iFrameWP - 1)*nLines;
    Receive(startPackage+startFrameWP+1).callMediaFunc = 1;
    for iLine = 1:nLines
      Receive(startPackage + startFrameWP + iLine).aperture = TX(iLine).aperture; % mux aperture same as transmit
      Receive(startPackage + startFrameWP + iLine).Apod(1:128) = 1.0;
      Receive(startPackage + startFrameWP + iLine).framenum = iPackage;
      Receive(startPackage + startFrameWP + iLine).acqNum = startFrameWP + iLine;
    end
  end
end
...
%  - Jump back to start.
SeqControl(1).command = 'jump'; scJump = 1;
SeqControl(1).argument = 1;
SeqControl(2).command = 'triggerIn'; scTriggerIn = 2;
SeqControl(2).condition = 'Trigger_1_Rising';
SeqControl(2).argument = 0; % wait forever for trigger to occur
SeqControl(3).command = 'triggerOut'; scTriggerOut = 3;
SeqControl(4).command = 'timeToNextAcq'; scTimeToNextAcqLines = 4;
SeqControl(4).argument = 200; % 400 usec
SeqControl(5).command = 'timeToNextAcq';  scTimeToNextAcqFrames = 5;
SeqControl(5).argument = 200;   % 10000 usec = 10msec time between frames
SeqControl(6).command = 'returnToMatlab'; scRetToMatlab = 6;
SeqControl(7).command = 'sync'; scSync = 7;
SeqControl(7).argument = 100000000; % 10 sec timeout for software sequencer (default is 0.5 seconds)
...
Event(n).recon = 0;
Event(n).proecss = 0;
Event(n).seqControl = [scSync, scTriggerIn];
n = n+1;
%}
...
for iPackage = 1:nPackages
  ...
  for iFrameWP = 1:nFramesWP
    startFrameWP = (iFrameWP - 1)*nLines;
    for iLine = 1:nLines        % Acquire frame
      if  iPackage == 1 && iFrameWP==1 && iLine == 1
        Event(n).info = 'acquire ray line.';
        Event(n).tx = iLine;   % use next TX structure.
        Event(n).rcv = startPackage + startFrameWP + iLine;
        Event(n).recon = 0;      % no reconstruction.
        Event(n).process = 0;    % no processing
        Event(n).seqControl = [scTriggerIn, scSync];
      else
        Event(n).info = 'acquire ray line.';
        Event(n).tx = iLine;   % use next TX structure.
        Event(n).rcv = startPackage + startFrameWP + iLine;
        ...
        Event(n).process = 0;    % no processing
        if iLine==nLines
          if iFrameWP == nFramesWP
            Event(n).seqControl = [scTimeToNextAcqFrames, nsc];
            SeqControl(nsc).command = 'transferToHost';
            nsc = nsc + 1;
          else
            Event(n).seqControl = scTimeToNextAcqFrames;
          end
        else
          Event(n).seqControl = scTimeToNextAcqLines;
        end  
      end
      n = n+1;
    end
...
Event(n).rcv = 0;        % no rcv
Event(n).recon = 0;      % no reconstruction
Event(n).process = 0;    % external processing
...
Event(n).info = 'Send OutTrigger';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 2;
Event(n).seqControl = [scTriggerOut, scRetToMatlab];
n = n+1;
...
% Save all the structures to a .mat file.
filename = [ filePath, '/', 'MatFiles', '/', 'L12-5_50mmFHFocus', '.mat'];
save(filename,'-v7.3');
